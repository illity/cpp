...
class Graph {
    ...
    void prim() {
        ...
        while(true) {
			...
			bool direction = (head == oldHead);
            newAdjacencyList[direction?oldTail:oldHead].insert(Edge(direction?tail:head, min));
            newAdjacencyList[direction?tail:head].insert(Edge(direction?oldTail:oldHead, min));
            visited.insert(direction?tail:head);
